                    ...
                    <div id="slider" class="nivoSlider">
                        @if(!empty($Car_news))
                            @foreach($Car_news as $item)
                                <a href="/post/more/{{$item->id}}"><img src="data:image/png;base64,{{$item->image}}"
                                                                        alt="2"
                                                                        title="{{$item->title}}"></a>
                            @endforeach
                        @else
                ...
                @foreach($allnews as $n)
                    @if($n->video == null)
                        <div style="{{$pad}}"><img src="data:image/png;base64,{{$n->image}}" alt="" width=240>
                            <span
                                class="data">{{Carbon\Carbon::parse($n->created_at)->formatLocalized('%d, %B %Y')}}</span>
    ...
    <div class="line1">
        <div class="line2">
            ...
            <div class="news_block">
                @foreach($allnews as $l)
                <div class="news_item">
                    <img src="http://127.0.0.1:8000/site/uploads/files/Main_carousel/03.jpg" alt="" width=240>
                    <span class="data">{{Carbon\Carbon::parse($l->created_at)->formatLocalized('%d, %B %Y')}}</span>
                    <a href="/post/more/{{$l->id}}" class=newlink><p>
                            {!! Str::words(strip_tags($l->title), $words = 9, $end = '...') !!}</p>
                    </a>
                </div>
                @endforeach
                <p style="text-align: center; margin-bottom: 24px;"><a href="/{{app()->getLocale()}}/news"
